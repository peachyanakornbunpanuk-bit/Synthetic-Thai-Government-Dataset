...
NAME_FILE = os.path.join(DRIVING_DIR, "assets", "name_surname", "thai_names_converted_TH_EN.txt")
SURNAME_FILE = os.path.join(DRIVING_DIR, "assets", "name_surname", "thai_surnames_TH_EN.txt")
...
TOTAL_IMAGES = 10  # <--- แก้จำนวนตรงนี้
...
def get_random_face(target_w, target_h):
    ...
    # ✅ กำหนด List โฟลเดอร์ที่ต้องการสุ่ม (ทั้งชายและหญิง)
    source_folders = [
        os.path.join(DRIVING_DIR, "data", "face_M"),  # โฟลเดอร์ผู้ชาย
        os.path.join(DRIVING_DIR, "data", "face_FM") # โฟลเดอร์ผู้หญิง
    ]
    
    all_face_paths = []

    # วนลูปเก็บไฟล์จากทุกโฟลเดอร์มารวมกัน
    for folder in source_folders:
        if os.path.exists(folder):
            try:
                files = [os.path.join(folder, f) for f in os.listdir(folder) 
                         if f.lower().endswith(valid_extensions)]
                all_face_paths.extend(files)
            except OSError:
                continue

    # ถ้าเจอรูป ให้สุ่มมา 1 รูป
    if all_face_paths:
        try:
            chosen_path = random.choice(all_face_paths)
            ...
            pass # ถ้าโหลดพัง ให้ไป return สีเทาด้านล่าง
            
    # ❌ Fallback: ถ้าไม่เจอรูปเลย หรือโหลดไม่ได้ ให้สร้างสีเทา
    print(f"⚠️ Warning: No face images found in {source_folders}")
...
def create_passport_v57(index):
    ...
    face_img = get_random_face(photo_w, photo_h)
    ...
    positions = {
        ...
        "mrz_line2": (22, 275, 16, "mrz", f"{pass_no}4THA{dob.strftime('%y%m%d')}8M{expiry.strftime('%y%m%d')}8{id_raw}00".ljust(44, '<')[:44])
    }
